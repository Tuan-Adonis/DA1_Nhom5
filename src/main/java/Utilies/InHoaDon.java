...
public class InHoaDon {
    ...
    public static boolean makePDF(HoaDonInRespone hoaDon, List<GioHangInRespone> gioHang) {
        ...
        try {
            ...
            for (int i = 0; i < gioHang.size(); i++) {
                ...
                billTable.addCell(getBillRowCell(gioHang.get(i).getDonGia().multiply(BigDecimal.valueOf(Double.parseDouble(String.valueOf(gioHang.get(i).getSoLuong())))) + ""));
            }
            ...
            validity.addCell(getValidityCell(" * Cửa hàng không chấp nhận đổi trả. Quý khách nên chọn lựa cẩn thận trước khi mua"));
            validity.addCell(getValidityCell(" "));
    ...
    public static PdfPTable tieuDe() throws Exception {
        ...
        fs1.addFont(font1);
        fs2.addFont(font2);
        Phrase p1 = fs1.process("CỬA HÀNG BÁN GIÀY THỂ THAO NAM\n"
                + "---------------------------------------------");
